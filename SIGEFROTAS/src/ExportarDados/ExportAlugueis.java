...
public class ExportAlugueis {
    
    private Connection conexao;
    public void ExportarAlugueis () throws InterruptedException {
        
        JOptionPane.showMessageDialog(null, "Caso você possua um arquivo \"PlanilhaLocacoes\" na sua \n Pasta de Downloads, remova-o para gerar o relatório");
        WritableWorkbook wworkbook;
        try {
            wworkbook = Workbook.createWorkbook(new File("/Users/matheusikenaga/Downloads/PlanilhaLocacoes.xls"));
            this.conexao = new ConnectionFactory().getConnection("root", "root1234");
            PreparedStatement ps=null;
            ResultSet rs=null;
            
            
            String query="select * from ALUG_VEIC_VIAGEM";
            ps= conexao.prepareStatement(query);
            System.out.println("Consulta no Banco:"+ps);
            rs=ps.executeQuery();
            WritableSheet wsheet = wworkbook.createSheet("Locacoes", 0);
            ...
            */
            ...
            while(rs.next()){
                ...
                wsheet.addCell(label);
                label = new Label(i++, j, selectMotAluguel(rs.getInt("CD_VIAGEM")));
                wsheet.addCell(label);
                label = new Label(i++, j, selectVeicAluguel(rs.getInt("CD_VIAGEM")));
                wsheet.addCell(label);
